...
namespace FirstGearGames.Mirrors.Assets.FlexNetworkAnimators
{
    ...
    public class FlexNetworkAnimator : NetworkBehaviour
    {
        ...
        private void CheckSendToServer()
        {
            ...
            {
                AnimatorUpdate au = ReturnAnimatorUpdate(updatedBytes);
                //Send to server.
                ArraySegment<byte> segment = CopyToBufferSegment(
                    (PooledNetworkWriter)Serialization.SerializeAnimatorUpdate(au),
                    ref _toServerBuffer);

                FlexNetworkAnimatorManager.SendToServer(segment);
            }
        }
        ...
        private void CheckSendToClients()
        {
            ...
            if (!sendFromServer)
            {
                ...
                for (int i = 0; i < _clientAuthoritativeUpdates.BufferCount; i++)
                {
                    ...
                    byte[] byteBuffer = _toClientsBuffer[i];
                    AnimatorUpdate au = ReturnAnimatorUpdate(new ArraySegment<byte>(buffer, 0, bufferLength));
                    ArraySegment<byte> segment = CopyToBufferSegment(
                        (PooledNetworkWriter)Serialization.SerializeAnimatorUpdate(au),
                        ref byteBuffer);

                    foreach (NetworkConnection conn in base.netIdentity.observers.Values)
                        FlexNetworkAnimatorManager.SendToObserver(conn, segment);
                }
            ...
            else
            {
                if (AnimatorUpdated(out ArraySegment<byte> updatedBytes))
                {
                    byte[] byteBuffer = _toClientsBuffer[0];
                    AnimatorUpdate au = ReturnAnimatorUpdate(updatedBytes);
                    ArraySegment<byte> segment = CopyToBufferSegment(
                        (PooledNetworkWriter)Serialization.SerializeAnimatorUpdate(au),
                        ref byteBuffer);

                        foreach (NetworkConnection item in base.netIdentity.observers.Values)
                            FlexNetworkAnimatorManager.SendToObserver(item, segment);
                }
            }
        }
        ...
        private bool AnimatorUpdated(out ArraySegment<byte> updatedBytes, bool forceAll = false)
        {
            ...
            foreach (KeyValuePair<int, StateChange> item in _unsynchronizedLayerStates)
            {
                ...
                else
                {
                    ...
                    Compressions.WriteCompressedFloat(_writer, sc.DurationTime);
                    Compressions.WriteCompressedFloat(_writer, sc.OffsetTime);
                    Compressions.WriteCompressedFloat(_writer, sc.NormalizedTransitionTime);
                }
            }
        ...
        private void ApplyParametersUpdated(ref ArraySegment<byte> updatedParameters)
        {
            ...
            try
            {
                using (PooledNetworkReader reader = (PooledNetworkReader)NetworkReaderPool.GetReader(updatedParameters))
                {
                    while (reader.Position < reader.Length)
                    {
                        ...
                        else if (parameterIndex == STATE)
                        {
                            ...
                            float normalizedTime = Compressions.ReadCompressedFloat(reader);
                            //Play results.
                        ...
                        else if (parameterIndex == CROSSFADE)
                        {
                            ...
                            //Get time values.
                            float durationTime = Compressions.ReadCompressedFloat(reader);
                            float offsetTime = Compressions.ReadCompressedFloat(reader);
                            float normalizedTransitionTime = Compressions.ReadCompressedFloat(reader);
                            //If using fixed.
                            if (useFixedTime)
        ...
        /// </summary>
        /// <param name="hash"></param>
        /// <param name="sendImmediately">True to send data the next frame. This is generally not needed but can be useful if you wish to send the trigger as animator parameters are currently rather than what they may be next interval.</param>
        public void SetTrigger(int hash, bool sendImmediately = false)
        {
            if (!_isActive)
                return;
            ...
            if (sendImmediately)
            {
                _nextClientSendTime = 0;
                _nextServerSendTime = 0;
            }
        }
        /// <summary>
        /// Sets a trigger on the animator and sends it over the network.
        ...
        /// <param name="hash"></param>
        /// <param name="sendImmediately">True to send data the next frame. This is generally not needed but can be useful if you wish to send the trigger as animator parameters are currently rather than what they may be next interval.</param>
        public void SetTrigger(string name, bool sendImmediately = false)
        {
            if (!_isActive)
                return;

            SetTrigger(Animator.StringToHash(name), sendImmediately);
        }
